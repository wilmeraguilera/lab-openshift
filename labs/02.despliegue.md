...
## 2.4 Despliegue aplicación desde código fuente desde el cliente oc
...
## 2.5 Despliegue aplicación desde Dockerfile

1.  Crear Build Config de tipo binario
```
oc new-build --strategy=docker --binary --name docker
```

2. Iniciar el build pasando como parámetro el directorio donde se encuentra el docker file y los artefactos necesarios para la creación de la imagen
```
 oc start-build docker --from-file=.
```

3. Crear la aplicación a partir de la imagen previamente creada
```
oc new-app -i docker
```

4. Exponer la aplicación mediante el uso de una ruta
```
oc expose svc docker
``` 



## 2.6 Replicas y Recuperación 
