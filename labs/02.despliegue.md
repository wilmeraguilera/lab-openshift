...
2. Seleccionar la opción **Container Image**
...
8. Volvemos a la Opción **Topology** y damos clic en la fecha superior derecha de la aplicación.
...
1. Crear un nuevo proyecto en Openshift llamado *apps-"#USERNAME#*, en mi caso el campo **Name** quedaria con el valor *apps-user1* los campos **Display Name** y **Description** pueden ser diligenciados con cualquier valor.
...
8. Dado que le entregamos a Openshift las fuentes de la aplicación, el paso que se ejecutará será el Build de la aplicación y posteriormente del Deploy. Vamos a la pantalla **Topology**, damos clic sobre la aplicación para ver el detalle y veremos que está corriendo el Build.
...
10. Cuando el Build termina debemos ver una salida en los logs similar a la siguiente imagen. Note que al final del log sale el mensaje **Push Sucessfull**, esto nos indica que la compilación de la imagen ya terminó y fue almacenada en el registry de openshift. 

![Logs Build finish](img/openjdk-template-build-logs-finish.png)

11. Luego vamos a ver que se crea un pod asociado al despliegue del API Rest cuyo estado debe ser **Running**. Al dar clic en **View Logs** vamos a evidenciar que inició la aplicación Spring por el puerto 8080.
...
1. Ubicarnos en el proyecto apps-*#USERNAME#* haciendo uso del siguiente comando. *Reemplazar la variable #USERNAME# con su usuario asignado.*
...
6. Tomar la ruta que en mi caso es http://lab-openshift-apps-user1.apps-crc.testing en un navegador. Debemos ver la pantalla de swagger del api.
