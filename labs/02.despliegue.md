...
Las cosas pueden ir mal con nuestra aplicación o por motivo externos, y por eso tenemos replicación y recuperación.
...
Esto permite recuperarnos rapidamente de una falla de un pod  y un equilibro de la carga adecuado entre los pods disponibles.

A continuación veremos como configurar replicas en Openshift con la aplicación desplegada en el numeral 2.2
* Vaya a la vista de __Topology__ y de clic en la aplicación openjdk-app.
* En la pestana de __Details__ puede observar que tenemos un pod en ejecución.

![replicacion-1-pod](img/replicacion-1-pod.png)

* De clic en la fecha hacia arriba para escalar de 1 pod a 3 pods.
* Vaya a la pestaña de __Resources__ y observará 3 Pods Activos.

![replicacion-resources-3-pods](img/replicacion-resources-3-pods.png)

* Si da clic en la ulr de la ruta observará que el comportamiento de la aplicación no ha cambiado (El api permanece desplegado). Con esta configuración el trafico será balanceado por el __Service__ entre los 3 pods activos.
...
* Navega hasta la vista de __Topology__ y haga clic sobre la aplicación openjdk-app.
* En el tab __Resources__ selecciona uno de los pods activos.
* Ya dentro del pod en el menú de acciones (__Actions__) vamos a eliminar el pod dando clic en __Delete Pod__ y confirmamos la eliminación.
* Rapidamente vaya a __Topology__, seleccione la aplicación y si fué lo suficientemente rápido en el tab de __Resources__ observará que un pod está siendo eliminado y otro se está creando.

![recovery-delete-pod](img/recovery-delete-pod.png)

* Si desea repita el ejercicio para ver el comportamiento.

### Salud de nuestras Aplicaciones
...
* Configure la aplicación para tener de nuevo un único pod atendiendo la peticiones y disminuir el consumo de recursos.
