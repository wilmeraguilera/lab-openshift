...
Ya dentro del pod conectarse a mysql con el siguiente comando. ```mysql -uroot -ppassword``` u seleccionar la base de datos prueba ```use prueba;``

Y proceder a crear la tabla.
...
2. En la consola web del Openshift buscar el config-map llamado myconfigmap, y adicionar el siguiente contenido. Estas propiedades nos permitirán que nuestra API se conecte a la base de datos Mysql.
...
El valor de {host} corresponde al Service llamado __mysql__ que fue previamente creado y que nos proporciona acceso a la base de datos.
...
Ingrese al pod de mysql haciendo uso del comando ```oc rsh ..```,  ya dentro del pod conéctese a la base de datos y verifique los nuevos registros en la tabla __users__.



### Secrets

En el ejercicio anterior los datos de usuario y contraseña de la base de datos están configurados en un config-map. Vamos a realizar unos pequeños ajustes para almacenar estos datos en un Secrets.

1. Crear el Secret en Openshift

```shell script
 oc create secret generic db-credentials  --from-literal USER_DB=root --from-literal PASS_DB=passwordSSSSSSSS
```
...
La salida que obtendrá será como la siguiente, observer que en la sección __data__ los campos *user* y *pass*  están codificados en base 64. 

```shell script
apiVersion: v1
data:
  pass: cGFzc3dvcmQ=
  user: cm9vdA==ssssssssss
kind: Secret
metadata:
  creationTimestamp: "2020-07-08T22:57:44Z"
  name: db-credentials
  namespace: apps-user1
  resourceVersion: "515994"
  selfLink: /api/v1/namespaces/apps-user1/secrets/db-credentials
  uid: 47d8bf97-3ab3-4f1f-8e4f-0c521e251b17
type: Opaque
```
3. Montar el secret en el deployment config como variables de entorno.
...
6. De manera opcional ingrese al pod de la aplicación y verifique las variables de entorno.
```shell script
oc rsh {nombre-pod}  
```

Ejecute los siguiente comando uno a uno para observar los valores
```shell script
echo $USER_DB
echo $PASS_DB
```
